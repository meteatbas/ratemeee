...
module.exports = (app) => {
    ...
    app.post('/company/register-employee/:id', (req, res, next) => {
        async.parallel([
            function(callback){
               Company.update({
                   '_id': req.params.id,//db'de bu id'ye sahip birisi varmı 
                   'employees.employeeId': {$ne: req.user._id}//not equal to inside employees arr check employee id is not equal to user id
               },
               {
                   //we push data into employees, req.body.role in html goes to employeeRole
                    $push: {employees: {employeeId: req.user._id, employeeFullname:req.user.fullname, employeeRole:req.body.role}}//req.user._id =session id,
               }, (err, count) => {
                   if(err){
                       return next(err);
                   }
            ...
            function(callback){//we are not passing count data here because we use "async.parallel" we dont need to send first function result into seconde function
                async.waterfall([
                    function(callback){//here we retrieve company data
                        Company.findOne({'_id': req.params.id}, (err, data) => {
                            callback(err, data);//we are passing err and result
                        })
                    },
    ...
    app.get('/companies/leaderboard', (req, res) => {
        Company.find({}, (err, result) => {
            res.render('company/leaderboard', {title: ' Companies Leaderboard', user: req.user, data: result});
        }).sort({'ratingSum':-1});
    });
    ...
    //name attribute is "search " in html company/search
